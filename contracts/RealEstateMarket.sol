...
contract RealEstateMarket {
    ...
    enum DealStage {
        ...
        PaymentReceived,   // 5 — деньги в escrow, ждёт вызова completeDeal
        RegistryPending,   // 6 — запрос в реестр отправлен, ждёт подтверждения
        Completed,         // 7 — сделка завершена
        Cancelled          // 8 — отменена
    }
    ...
    function reservePayment(
        ...
        _emitStage(dealId, unicode"Оплата получена в escrow. Запрос к реестру для оформления сделки...");

        // Сразу запрашиваем реестр — без дополнительного шага от пользователя
        ...
        _emitStage(dealId, unicode"Реестр обрабатывает оформление права собственности...");
    }
    ...
    function onRegistryTransferConfirmed(
        ...
        if (success) {
            ...
            _emitStage(dealId, unicode"Реестр подтвердил! Право собственности оформлено. Перевод средств продавцу...");

            // Переводим деньги продавцу
            uint256 amount = address(this).balance;
            // Защита: переводим только цену сделки, не весь баланс контракта
            uint256 toSend = d.price < amount ? d.price : amount;
            (bool sent, ) = payable(d.seller).call{value: toSend}("");
            require(sent, "Transfer to seller failed");

            emit FundsRefunded(dealId, d.seller, toSend);
        } else {
            ...
            uint256 amount = address(this).balance;
            uint256 toRefund = d.price < amount ? d.price : amount;
