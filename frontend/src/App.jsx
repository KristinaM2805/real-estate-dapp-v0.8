...
const STAGES = [
  "Данные продавца подготовлены",
  "Oracle проверяет продавца",
  "Сделка создана и опубликована",
  "Покупатель подал данные",
  "Покупатель верифицирован",
  "Оплата получена в escrow",
...
export default function App() {
  ...
  const sellerNextActionMessage = useMemo(() => {
    ...
    if (stage === 5) return "Покупатель внёс оплату в escrow. Ожидаем запрос на переоформление права собственности в реестре.";
    if (stage === 6) return "Идёт переоформление права собственности в реестре. Oracle отправит результат в смарт-контракт автоматически.";
    return null;
  ...
  const simpleDealMessage = useMemo(() => {
    ...
    if (stage === 5) return `${idText} Оплата внесена в escrow. Ожидаем запрос на переоформление.`;
    if (stage === 6) return `${idText} Реестр регистрирует нового владельца. Ожидаем финальное подтверждение Oracle.`;
    if (stage === 7) return `${idText} Сделка завершена: новый владелец зарегистрирован, средства переведены продавцу.`;
    if (stage === 8) return `${idText} Сделка отменена.`;
    return message;
 ...
 const visualStage = isCancelled ? 0 : (replayStage ?? stage);
  const progressStage = isCancelled ? stage : (replayStage ?? stage);
  ...
  async function loadLastDealForAddress(c, address) {
    ...
    try {
      ...
      for (let i = 0; i < count; i++) {
        try {
          const d = await readDeal(c, i);
          const isActive = d.stage < 7 && !isCancelledDeal(d);
  ...
  useEffect(() => {
    ...
    if (!contract || idToPoll === null) return;

    const shouldPoll = !deal || (Number(deal.stage) !== 7 && Number(deal.stage) !== 8);

    if (shouldPoll) {
      pollRef.current = setInterval(async () => {
        const latest = await loadDeal(contract, idToPoll, { openDeal: true });
        if (latest?.stage === 7 || latest?.stage === 8) {
          clearInterval(pollRef.current);
        }
  ...
  async function handleSubmitBuyerData() {
    ...
    try {
      ...
      for (let i = 0; i < 12; i++) {
        ...
        await loadMyDeals(c, address);
        if (latest?.stage >= 4 || latest?.stage === 8) break;
      }
    } catch (e) {
      console.error(e);
      setMmStatus("error");
  ...
  async function handleReservePayment() {
    ...
    try {
      ...
      setMmStatus("waiting");
      setShowPaymentConfirm(false);
      setMessage("Подтвердите оплату в MetaMask. После оплаты реестр начнёт переоформление...");
      ...
      setMmStatus("success");
      mmTimer.current = setTimeout(() => setMmStatus("idle"), 3000);
      setMessage("💰 Оплата внесена в escrow. Ожидаем переоформление права собственности в реестре...");
      setAnimPhase(p => p + 1);
  ...
  const oracleLabel = stage === 1 ? "⏳ Oracle проверяет продавца перед созданием активной сделки..."
    ...
    : stage === 6 ? "⏳ Реестр регистрирует нового владельца..."
    : null;
  ...
  return (
    <main className="page">
      ...
      <aside className="controls controls--side">
        ...
        {isSeller && deal && stage >= 2 && stage < 7 && !isCancelled && (
          <div className="nameInputBox" style={{ gridColumn: "1/-1" }}>
            ...
            <p style={{ margin: "6px 0 0", color: "#475569", fontSize: 13, lineHeight: 1.45 }}>
              ...
              {stage === 5 && `Покупатель ${buyerDisplayName} внёс оплату в escrow. Средства заблокированы в смарт-контракте.`}
              {stage === 6 && "Oracle переоформляет право собственности в реестре. Действия продавца не требуются."}
            </p>
            {hasBuyer && (
              ...
              return (
                ...
                >
                  ...
                  <span className="action-btn__text">
                    ...
                    <small>{itemRole} · {STAGES[item.stage] || "этап неизвестен"} · {ethers.formatEther(item.price)} ETH</small>
                  </span>
                </button>
              );
            })}
